...
if TYPE_CHECKING:
    ...
    from gdansk.resource_meta import ResourceMeta
...
def test_widget_resource_meta_overrides_domain_and_augments_csp(views_path: Path):
    ship_resource_meta: ResourceMeta = {
        "openai/widgetDescription": "Shared widget description",
        "ui": {
            "csp": {
    ...
    widget_resource_meta: ResourceMeta = {
    ...
    ship = Ship(
        views=views_path,
        base_url="https://example.com/app",
        resource_meta=ship_resource_meta,
    )

    @ship.widget(path=Path("hello/widget.tsx"), name="hello", resource_meta=widget_resource_meta)
    def hello() -> None:
        return None
...
def test_widget_rejects_invalid_resource_meta_csp_domain(views_path: Path):
    ...
    with pytest.raises(ValueError, match=r"ui\.csp\.connectDomains must be an absolute URL with a hostname"):
        ...
        @ship.widget(
            ...
            name="hello",
            resource_meta={"ui": {"csp": {"connectDomains": ["/relative"]}}},
        )
        def hello() -> None:
            return None
