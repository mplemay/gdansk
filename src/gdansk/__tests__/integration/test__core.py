...
def test_runtime_imports_pyproject_gdansk_dependency(tmp_path: Path):
...
""",
    ...
    script = Script('import { join } from "std_path"; export default () => join("a", "b");')
    with Runtime(cwd=tmp_path)(script) as run:
        assert run() == "a/b"
