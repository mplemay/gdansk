...
pub(crate) struct PackageEnvironment {
    inner: Arc<PackageEnvironmentInner>,
}
...
impl PackageEnvironment {
    ...
    fn from_dependencies(
        ...
        Ok(Self {
            inner: Arc::new(PackageEnvironmentInner {
                cwd,
                config_file,
                lockfile,
                dependencies,
    ...
    pub(crate) fn cli_config_options(&self) -> deno_cli::python::PythonPackageConfigOptions {
        deno_cli::python::PythonPackageConfigOptions {
            cwd: self.cwd().to_path_buf(),
            config_file: self.config_file().to_path_buf(),
            lockfile: self.lockfile().to_path_buf(),
        }
    }
}
...
pub(crate) async fn install_packages(
    ...
    deno_cli::python::install_packages(deno_cli::python::PythonInstallOptions {
        config: cli_config_options(&env),
        lockfile_only,
    })
    .await?;
...
pub(crate) async fn update_packages(
    ...
    deno_cli::python::update_packages(deno_cli::python::PythonUpdateOptions {
        config: cli_config_options(&env),
        filters: packages,
        latest,
        lockfile_only,
...
fn cli_config_options(env: &PackageEnvironment) -> deno_cli::python::PythonPackageConfigOptions {
    env.cli_config_options()
}
...
fn collect_table_deps(
    ...
    let Some(table) = document
        .get("gdansk")
        .and_then(|gdansk| gdansk.get(table_name))
...
fn apply_updates_to_pyproject(
    ...
    let imports = config
        ...
        .ok_or_else(|| anyhow!("Synthetic Deno config is missing an imports table"))?;
...
mod tests {
    ...
    #[test]
    fn reads_pyproject_gdansk_dependency_tables() {
