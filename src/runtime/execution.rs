...
fn run_worker_thread(bound: BoundRuntime, receiver: mpsc::Receiver<ExecutionCommand>) {
    ...
    let mut context = match DenoExecutionContext::new(bound) {
        Ok(context) => context,
        Err(error) => {
...
enum ExecutionRuntime {
    Core(JsRuntime),
    CliPending(PackageEnvironment),
    Cli {
        worker: Box<deno_cli::python::PythonMainWorker>,
        _package_environment: PackageEnvironment,
    },
}

impl DenoExecutionContext {
    fn new(bound: BoundRuntime) -> ExecutionResult<Self> {
        let main_module = main_module_specifier(&bound)?;
        let runtime = match PackageEnvironment::discover(bound.cwd().to_path_buf(), true)
            .map_err(|error| BindingError::runtime(error.to_string()))?
        {
            Some(package_environment) => ExecutionRuntime::CliPending(package_environment),
            None => ExecutionRuntime::Core(create_js_runtime(&bound)?),
        };
        Ok(Self {
            bound,
            main_module,
    ...
    async fn invoke(&mut self, arguments: RunnerArguments) -> ExecutionResult<PyJsValue> {
        ...
        let js_runtime = self.js_runtime();
        let call = js_runtime.call_with_args(&run_function, &args);
    ...
    async fn ensure_loaded(&mut self) -> ExecutionResult<()> {
        ...
        let module_id = self.load_main_module().await?;
    ...
    async fn ensure_runtime(&mut self) -> ExecutionResult<()> {
        let ExecutionRuntime::CliPending(package_environment) = &self.runtime else {
            return Ok(());
        };
        let package_environment = package_environment.clone();
        let worker =
            deno_cli::python::create_main_worker(deno_cli::python::PythonMainWorkerOptions {
                config: package_environment.cli_config_options(),
                main_module: self.main_module.clone(),
                source: Some(self.bound.script().content().to_string()),
            })
            .await
            .map_err(|error| BindingError::runtime(error.to_string()))?;
        self.runtime = ExecutionRuntime::Cli {
            worker: Box::new(worker),
            _package_environment: package_environment,
        };
        Ok(())
    }

    async fn load_main_module(&mut self) -> ExecutionResult<ModuleId> {
        let main_module = self.main_module.clone();
        match &mut self.runtime {
            ExecutionRuntime::Core(js_runtime) => js_runtime
                ...
                .await
                .map_err(|error| BindingError::module_load(error.to_string())),
            ExecutionRuntime::Cli { worker, .. } => worker
                .js_runtime()
                .load_main_es_module(&main_module)
                .await
                .map_err(|error| BindingError::module_load(error.to_string())),
            ExecutionRuntime::CliPending(_) => unreachable!(),
        }
    }

    async fn run_event_loop(&mut self) -> ExecutionResult<()> {
        match &mut self.runtime {
            ExecutionRuntime::Core(js_runtime) => js_runtime
                .run_event_loop(Default::default())
                .await
                .map_err(map_core_error),
            ExecutionRuntime::Cli { worker, .. } => {
                worker.run_event_loop(false).await.map_err(map_core_error)
            }
            ExecutionRuntime::CliPending(_) => unreachable!(),
        }
    }
    ...
    fn js_runtime(&mut self) -> &mut JsRuntime {
        match &mut self.runtime {
            ExecutionRuntime::Core(js_runtime) => js_runtime,
            ExecutionRuntime::Cli { worker, .. } => worker.js_runtime(),
            ExecutionRuntime::CliPending(_) => unreachable!(),
        }
    }
}
...
fn inline_module_path(bound: &BoundRuntime) -> PathBuf {
    bound.cwd().join("__gdansk_inline__.ts")
}
