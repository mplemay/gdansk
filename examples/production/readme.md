...
Production builds only `dist/gdansk-manifest.json`; there is no static widget asset mount or separate JS runtime server.
Development uses `ship.mcp(..., watch=True)` to build inline HTML on startup and rebuild when widget files change.
