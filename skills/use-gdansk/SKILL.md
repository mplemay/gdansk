...
- Widget resource metadata:
  - Use `Ship(..., widget_meta=...)` for shared widget resource `_meta` like widget CSP or domain.
  - Override per-widget widget resource metadata with `@ship.widget(..., widget_meta=...)`.
- Structured outputs:
