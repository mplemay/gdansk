...
| --- | --- | --- | --- |
| `The frontend root directory ... does not exist` | `Vite(...)` points at a missing directory | Point `Vite(...)` at the frontend package root that contains `package.json` and `widgets/` | Inspect the server entrypoint and confirm the resolved path |
| `The frontend root directory ... is not a directory` | `Vite(...)` points at a file | Pass the package directory instead of a file path | Inspect the `Vite(...)` argument |
...
| `The frontend dev server did not start in time` | Vite did not boot, or Python and Vite disagree on host or port | Fix the Vite startup issue and keep `Vite(Path(...), host=..., port=...)` and `gdansk({ host, port })` aligned | Check `vite.config.ts`, package dependencies, and the configured host/port on both sides |
