...
| `The frontend dev server did not start in time` | Vite did not boot, or Python and Vite disagree on host or port | Fix the Vite startup issue and keep `Vite(host, port)` (via `Ship(..., vite=...)`) and `gdansk({ host, port })` aligned | Check `vite.config.ts`, package dependencies, and the configured host/port on both sides |
